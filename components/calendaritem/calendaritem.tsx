...
>((props, ref) => {
  ...
  const weeksPanel = useRef<View|null>(null);
  const monthsScroll = useRef<ScrollView|null>(null);
  const monthsPanel = useRef<View|null>(null);
  const viewArea = useRef<View|null>(null);
  ...
  const chooseDay = (day: Day, month: MonthInfo, isFirst?: boolean) => {
    if (getClass(day, month) !== 'disabled') {
      const { type } = props;
  ...
  const initData = () => {
    ...
    requestAniFrame(() => {
      // 初始化 日历位置
      if (monthsScroll && monthsPanel && viewArea) {
        (monthsScroll.current as any)?.measure((x, y, width, height, pageX, pageY)=>{
            viewHeight = height;
            monthsPanel.current?.setNativeProps({ style: { height: containerHeight } })
            monthsScroll.current?.scrollTo({ y: state.monthsData[state.currentIndex].cssScrollHeight, animated: false })
        })
      }
    })
  ...
  const scrollToDate = (date: string) => {
    ...
    state.monthsData.forEach((item, index) => {
      if (
        ...
        if (monthsScroll.current) {
          const distance = state.monthsData[index].cssScrollHeight - scrollTop.current;
          if (toDateAnimation) {
            let flag = 0;
            const interval = setInterval(() => {
              flag++;
                const offset = distance / 10;
                monthsScroll.current?.scrollTo({y: scrollTop.current + offset, animated: false})

              if (flag >= 10) {
                clearInterval(interval);
                monthsScroll.current?.scrollTo({y: state.monthsData[index].cssScrollHeight, animated: false})
              }
            }, 40);
          } else {
            monthsScroll.current?.scrollTo({y: state.monthsData[index].cssScrollHeight, animated: false})
  ...
  return (
    <View style={styles.calendar}>
      ...
      <View style={styles.calendarHeader}>
        ...
        {onBtn && <View style={styles.calendarTopSlot}>{onBtn()}</View>}
        {showSubTitle && (
          <Text style={styles.calendarCurrMonth}>{yearMonthTitle}</Text>
        )}
      ...
      <ScrollView
        ...
        ref={monthsScroll}
      >
        <View style={styles.calendarMonthsPanel} ref={monthsPanel}>
          <View
            ref={viewArea}
            style={{
              transform: [
              ...
              .map((month: any, key: number) => {
                return (
                  <View style={styles.calendarMonth} key={key}>
                    ...
                    <View style={styles.calendarMonthItem}>
                      {month.monthData.map((day: Day, i: number) => (
                        <TouchableOpacity
                          activeOpacity={1}
                          style={[
                            styles.calendarMonthDay,
                            styles[`calendarMonthDay__${getClass(day, month)}`],
                          ]}
                          onPress={() => {
                            chooseDay(day, month);
                          }}
                        ...
                        >
                          <Text style={[
                            styles.calendarDay,
                            (i%7 === 0 || i%7===6) && day.type ==='curr' && styles.calendarMonthConDay,
                            styles[`calendarDay__${getClass(day, month)}`],
                          ]}>
                          ...
                          {onTopInfo && (
                            <Text
                              style={[
                                styles.calendarCurrTips,
                                styles.calendarCurrTipsTop,
                              ]}
                            >
                              {onTopInfo(day)}
                            </Text>
                          ...
                          {onBottomInfo && (
                            <Text
                              style={[
                                styles.calendarCurrTips,
                                styles.calendarCurrTipsBottom,
                                styles[`calendarCurrTips__${getClass(day, month)}`],
                              ]}
                            >
                              {onBottomInfo(day)}
                            </Text>
                          ...
                          {!onBottomInfo &&
                            ...
                            isCurrDay(month, day.day) && (
                              <Text style={[
                                    styles.calendarCurrTipCurr,
                                    styles[`calendarCurrTipCurr__${getClass(day, month)}`],
                                ]}>
                          ...
                          {isStartTip(day, month) && (
                            <Text
                              style={[
                                styles.calendarDayTip,
                                rangeTip() && styles.calendarCurrTipsTop,
                                styles[`calendarDayTip__${getClass(day, month)}`],
                              ]}
                            >
